...
`;
  ...
  console.log(`\x1b[33m✅ CREATED: available-sfsymbols.ts with ${sortedNames.length} symbols (SOURCE OF TRUTH)\x1b[0m`);
}
...
`;
  ...
  fs.writeFileSync(filePath, fileContent);
  console.log(`\x1b[33m✅ CREATED: icons-data.ts with ${Object.keys(iconNamesAndContent).length} icons\x1b[0m`);
}
...
`;
  ...
  fs.writeFileSync(filePath, fileContent);
  console.log(`\x1b[33m✅ CREATED: icons-map.ts with ${sortedNames.length} mappings\x1b[0m`);
}
...
`;
  ...
  fs.writeFileSync(filePath, fileContent);
  console.log(`📝 Generated: index.tsx with ${sortedNames.length} exports`);
}
...
async function generateSFIcons() {
  ...
  if (svgFiles.length === 0) {
    console.log('⚠️  No SVG files found in svgs directory.');
    console.log('ℹ️  Checking for existing generated files...');
    ...
    if (fs.existsSync(existingEnumFile)) {
      ...
      if (enumMatch) {
        ...
        console.log(`📊 Found ${iconCount} existing symbols in generated files`);
        updateReadmeBadge(iconCount);
        return;
      }
    }

    console.log('⚠️  No existing generated files found either.');
    updateReadmeBadge(0);
    return;
  }
  ...
  svgFiles.forEach(svgFile => {
    ...
    try {
      ...
      console.log(`\x1b[33m✅ PROCESSED: ${svgFile} → ${iconName}\x1b[0m`);
    } catch (error) {
      console.error(`❌ ERROR processing ${svgFile}:`, error instanceof Error ? error.message : error);
    }
  });
  ...
  console.log(`\n📊 Summary: ${Object.keys(iconNamesAndContent).length} icons processed\n`);
}
...
function updateReadmeBadge(iconCount: number): void {
  ...
  console.log(`\x1b[33m✅ UPDATED: README.md badge with ${iconCount} symbols\x1b[0m`);
}
